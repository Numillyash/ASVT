    ...
    if(!hadRecievedMessage){
        switch (event) {
            ...
            case I2C_SLAVE_FINISH: // master has signalled Stop / Restart
                ...
                int isMessageStart = context.buffer[0] == 0 ? 1 : 0;
                int isMessageEnd = context.buffer[0] == 2 ? 1 : 0;
                if(isMessageStart)
                {
                ...
                else
                {
                    ...
                    printf("%s\n", context.mem);
                    context.mem_address = context.buffer_address - 2;
                }
...
void pwm_interrupt_handler() {
    pwm_clear_irq(pwm_gpio_to_slice_num(LED_PIN));    
        pwm_set_gpio_level(LED_PIN, context.mem[context.mem_address]*10);  
        context.mem_address++;
}
